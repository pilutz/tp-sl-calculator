...
try{
 ...
 const download3=page.waitForEvent('download');await page.click('#exportBtn');const dl3=await download3;const file3=await dl3.path();const exported3=JSON.parse(fs.readFileSync(file3,'utf8'));assert.equal(exported3.version,'3.1.0');assert.equal(exported3.source.ocr.length,3);assert.equal(exported3.executionVolume,0);assert.equal(exported3.ordersSent,false);assert.equal(exported3.checks.position,'yes');pass('Three-photo export keeps provenance and never sends an order');
