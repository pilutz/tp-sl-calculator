...
try{
 ...
 await page.waitForFunction(()=>document.querySelector('#sourceStatus').textContent.includes('140 rânduri'));await page.fill('#symbol','RANGE TEST');await page.selectOption('#timeframe','M15');await page.fill('#tick','.01');await page.check('#dataConfirmed');await page.click('#analyzeBtn');assert.equal(await page.locator('#signal').innerText(),'FĂRĂ SETUP');assert.doesNotMatch(await page.locator('#actionStatus').innerText(),/Completează/);pass('Sideways data gives no fabricated trade');
 await page.setInputFiles('#csvInput',{name:'short.csv',mimeType:'text/csv',buffer:Buffer.from(csv(TP.demo('SHORT')))});
 await page.waitForFunction(()=>document.querySelector('#sourceStatus').textContent.startsWith('short.csv'));await page.fill('#symbol','SHORT TEST');await page.selectOption('#timeframe','M15');await page.fill('#tick','.01');await page.check('#dataConfirmed');await page.click('#analyzeBtn');assert.match(await page.locator('#signal').innerText(),/SELL SHORT/);pass('CSV automatically generates short without manual direction selector');
 const longHistory=TP.demo('LONG',4000);await page.setInputFiles('#csvInput',{name:'long-history.csv',mimeType:'text/csv',buffer:Buffer.from(csv(longHistory))});
 await page.waitForFunction(()=>document.querySelector('#sourceStatus').textContent.startsWith('long-history.csv'));await page.fill('#symbol','ISTORIC TEST');await page.selectOption('#timeframe','M15');await page.fill('#tick','.01');await page.check('#dataConfirmed');await page.click('#analyzeBtn');
 assert.match(await page.locator('#signal').innerText(),/BUY LONG/);assert.match(await page.locator('#statsOut').innerText(),/Cazuri activate/);assert.match(await page.locator('#statsOut').innerText(),/Interval Wilson 95%/);pass('Long CSV runs chronological holdout backtest with Wilson interval');
 ...
 await page.waitForFunction(()=>document.querySelector('#sourceStatus').textContent.includes('fotografii încărcate'));
 ...
 assert.equal(await page.locator('#directionRows tr').count(),2);assert.match(await page.locator('#compareSummary').innerText(),/Direcția confirmată/);pass('Both LONG and SHORT are compared before selection');
 const download=page.waitForEvent('download');await page.click('#exportBtn');const dl=await download;const file=await dl.path();const exported=JSON.parse(fs.readFileSync(file,'utf8'));assert.equal(exported.version,'3.3.0');assert.equal(exported.executionVolume,0);assert.equal(exported.ordersSent,false);assert.equal(exported.source.approximate,true);assert.equal(exported.analysis.direction,'LONG');pass('Export includes provenance, calibration and zero automatic execution');
 // Mock integration tests the 3-photo OCR/reconciliation pipeline, not recognition accuracy.
 const photoBuffers=[1,2,3].map(i=>({name:'xtb-'+i+'.png',mimeType:'image/png',buffer:Buffer.from(fixture.url.split(',')[1],'base64')}));
 await page.evaluate(({hi,lo})=>{let call=0;const texts=[
  'OIL CFD M15 106.16 USD 22:18 Cumparare 106.22 Vanzare 106.16 -0.005 -40.01 RON 104.54',
  'OIL CFD M15 106.21 USD Astazi 1.73% Volum 0.005 Marja = 241.32 RON Fonduri libere: 559.72 RON Sell 106.21 Buy 106.27 Spread: 1.37 RON / 0.06 PIPS Commission 0.00 RON Valoare contract = 531.20 USD / = 2431.30 RON',
  'OIL CFD M15 106.22 USD Astazi 1.74% Volum 0.005 Marja = 241.32 RON Fonduri libere: 558.34 RON Sell 106.22 Buy 106.28 Spread: 1.37 RON / 0.06 PIPS Commission 0.00 RON Valoare contract = 531.25 USD / = 2431.53 RON Swap zilnic Sell -0.53 RON / Buy -1.01 RON Valoare pip = 22.88 RON Faza pietei Deschis'
 ];const blocks=[{paragraphs:[{lines:[{words:[
  {text:Number(hi).toFixed(2),confidence:99,bbox:{x0:1120,x1:1175,y0:20,y1:34}},
  {text:Number((hi+lo)/2).toFixed(2),confidence:99,bbox:{x0:1120,x1:1175,y0:218,y1:232}},
  {text:Number(lo).toFixed(2),confidence:99,bbox:{x0:1120,x1:1175,y0:416,y1:430}}
 ]}]}]}];window.Tesseract={createWorker:async()=>({setParameters:async()=>{},terminate:async()=>{},recognize:async()=>{const idx=Math.min(2,Math.floor(call/2)),withBlocks=call%2===0;call++;return {data:{text:texts[idx],blocks:withBlocks?blocks:[]}};}})};},fixture);
 await page.setInputFiles('#imageInput',photoBuffers);await page.waitForFunction(()=>document.querySelectorAll('#photoList .photo-item').length===3);
 await page.click('#scanAllBtn');await page.waitForFunction(()=>document.querySelector('#sourceStatus').textContent.includes('s-a încheiat'),null,{timeout:30000});
 assert.match(await page.locator('#extracted').innerText(),/Valoare calculată \/ lot/);assert.equal(Number(await page.inputValue('#value')),4576);assert.equal(Number(await page.inputValue('#marginPerLot')),48264);assert.equal(Number(await page.inputValue('#freeMargin')),558.34);assert.equal(await page.inputValue('#position'),'yes');assert.equal(await page.inputValue('#market'),'open');
 assert.match(await page.locator('#reconcileStatus').innerText(),/incompatibil cu SL pentru SHORT/);assert.equal(await page.isChecked('#dataConfirmed'),false);assert.match(await page.locator('#signal').innerText(),/BUY LONG/);assert.match(await page.locator('#sourceBadge').innerText(),/1–3 FOTO/);assert.match(await page.locator('#blockers').innerText(),/Poziție existentă/);pass('Three photos are OCR-parsed, reconciled and auto-propose a blocked conditional setup');
 const download3=page.waitForEvent('download');await page.click('#exportBtn');const dl3=await download3;const file3=await dl3.path();const exported3=JSON.parse(fs.readFileSync(file3,'utf8'));assert.equal(exported3.version,'3.3.0');assert.equal(exported3.source.ocr.length,3);assert.equal(exported3.executionVolume,0);assert.equal(exported3.ordersSent,false);assert.equal(exported3.checks.position,'yes');pass('Three-photo export keeps provenance and never sends an order');
